...
import csv
import socket
import collections # <--- New import for deque
...
CHUNK_SIZE = 20 # <--- MATCH THIS TO YOUR TRAINING CONFIG
...
# This queue holds the future actions. 
# The AI fills it, the Sender drains it.
...
# ---------------------------------------------------------------------------
# UPDATED AI MODEL (Matches new chunk_size)
# ---------------------------------------------------------------------------
class SentryPolicy(nn.Module):
    def __init__(self):
        super().__init__()
        self.chunk_size = CHUNK_SIZE # Uses global config (30)
    ...
    def __init__(self):
        ...
        if os.path.exists(SENTRY_MODEL_PATH):
            try:
                ...
                self.model.eval()
                print(f"SUCCESS: Loaded Sentry AI")
            except Exception as e:
                print(f"ERROR: Failed to load AI: {e}")
        else:
            print(f"WARNING: Model not found at {SENTRY_MODEL_PATH}")

    def decide(self, frame) -> List[Tuple[float, float]]:
        """Returns A LIST of actions (the full chunk)"""
        if self.model is None or frame is None:
            return []

        curr_img = cv2.resize(frame, (240, 240))
        curr_img = cv2.cvtColor(curr_img, cv2.COLOR_BGR2RGB)
        
        if self.prev_frame is None:
            self.prev_frame = curr_img 

        stacked_img = np.concatenate([self.prev_frame, curr_img], axis=2)
        self.prev_frame = curr_img 

        img_tensor = torch.tensor(stacked_img, dtype=torch.float32).permute(2, 0, 1) / 255.0
        ...
        # Reshape flat array [L0, R0, L1, R1...] into list of tuples
        actions = []
        for i in range(0, len(flat), 2):
            actions.append((float(flat[i]), float(flat[i+1])))
...
# ---------------------------------------------------------------------------
# UPDATED SENDER LOOP (The Consumer)
# ---------------------------------------------------------------------------
def send_command(left, right):
    try:
        if USE_UDP and udp_sock:
            payload = f"{left:.2f},{right:.2f}".encode()
...
def sender_loop():
    """
    Consumes actions from the action_queue at a fixed rate (30Hz).
    """
    ...
    while sender_running.is_set():
        ...
        if current_mode == AppMode.SENTRY:
            # SENTRY MODE: Consume from Buffer
            with queue_lock:
                if len(action_queue) > 0:
                    # Pop the next immediate action
                    target_l, target_r = action_queue.popleft()
                    current_action = [target_l, target_r] # Update global for HUD
                else:
                    # Buffer empty! Robot has outrun its brain. Stop safely.
                    target_l, target_r = 0.0, 0.0
        else:
            # TELEOP/TRACKING: Just use the global current_action immediately
        ...
        # Maintain strict 30Hz timing for the robot
...
def main():
    ...
    while True:
        ...
        if current_mode == AppMode.SENTRY:
            ...
            # 2. OVERWRITE BUFFER LOGIC
            # If we get a new chunk, we trust it more than the old queue.
            if new_chunk:
                with queue_lock:
                    action_queue.clear()         # <--- Dump old actions
                    action_queue.extend(new_chunk) # <--- Load fresh actions
