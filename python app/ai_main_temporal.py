...
DRIVE_SCALE = 0.2  # scale outgoing drive commands
MAX_FPS = 30
...
# ---------------------------------------------------------------------------
# AI Model Definitions (MUST MATCH TRAIN SCRIPT)
# ---------------------------------------------------------------------------
class SentryPolicy(nn.Module):
    """
    The ResNet Policy Architecture.
    STRICTLY MATCHES 'train_temporal.py'
    """
    def __init__(self):
        super().__init__()
        self.chunk_size = 20 
        ...
        # 2. CRITICAL: Modify First Layer for 6 Channels (Stack of 2 images)
        old_conv = self.backbone.conv1
        self.backbone.conv1 = nn.Conv2d(
            in_channels=6, # <--- Expects 6 channels now
        ...
        self.head = nn.Sequential(
            ...
            nn.Tanh() # <--- Matches training script
        )
...
class SentryBrain:
    def __init__(self):
        ...
        self.model = None
        self.prev_frame = None # Memory for temporal stacking
    ...
    def decide(self, frame) -> Tuple[float, float]:
        ...
        # 2. Handle First Frame (Cold Start)
        if self.prev_frame is None:
            self.prev_frame = curr_img 

        # 3. Stack Frames (Memory + Current)
        # Result is (240, 240, 6)
        ...
        # Normalize with 6-channel stats
        ...
        # 6. Action (already in -1 to 1 due to Tanh)
